...
| 使い方マニュアル | 追加済み | 追加済み | 未作成 | ☐ |
| 動画視聴・メモ・質問 | 未着手 | 未着手 | 未作成 | ☐ |
| YouTube検索・登録・カテゴリ | 追加済み | 追加済み | 未作成 | ☐ |
...
| コミュニティ運営 | 参加申請一覧・承認・却下と監査記録を実装・単体テスト済み | 参加申請一覧・承認・却下と監査記録を実装・単体テスト済み | Owner、委譲権限、旧管理者データの互換判定を両OSで検証。権限編集・その他運営機能は後続タスク | ☐ |
| 公開投稿・フォロー・ファン | 未着手 | 未着手 | 未作成 | ☐ |
| 有料運営プラン | 未着手 | 未着手 | 未作成 | ☐ |
| アカウント・セキュリティ | 開発用Firebase接続・会員情報保存・生体認証再ログインを実装・単体テスト済み | 開発用Firebase接続・会員情報保存・Face ID/Touch ID再ログインを実装・単体テスト済み | 名前・ふりがな・メール・パスワードの登録検証、メール認証、ログイン、再設定、`memberPrivate/{uid}`互換保存を両OSで検証。パスワードは保存せず、更新トークンをKeychain／Keystore暗号化領域へ保存。ログアウト・退会は後続タスク | ☐ |
