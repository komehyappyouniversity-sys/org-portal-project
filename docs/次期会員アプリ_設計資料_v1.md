...
| お気に入り | Webページの端末内CRUD、カテゴリ・メモ、OS共有、共通JSONバックアップ | 4.2、G12 |
...
Android・iPhoneで共通のJSON形式（`format: org-portal-favorites-backup`、`version: 1`）を使用し、両OS間でも読み込み可能とする。読み込みはID単位の追加・更新として扱い、読み込み前から端末にある別のお気に入りは削除しない。バックアップを書き出さずにアンインストールした場合、削除済みの端末内データは復元できない。会員アカウントを使った自動クラウド同期・自動復元は、会員登録後のFirebase移行と合わせて別タスクで実装する。
...
| お気に入り | 実装・単体テスト済み | 実装・単体テスト済み | タイトル・URL・メモ・カテゴリの端末内CRUD、リンク起動、OS共有、共通JSONバックアップを両OSで検証 | ☑ |
