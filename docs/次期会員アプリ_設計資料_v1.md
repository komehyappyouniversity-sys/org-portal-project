...
- 予定・今日の予定
- 日記・写真日記
- 金種計算（レジ精算などで紙幣・硬貨の枚数から合計金額を計算する機能）
- 会議録音・議事録
- SNS共有
- お気に入り
...
**追加要件**: 一覧・詳細・作成・編集・削除／写真拡大表示／本人だけが閲覧可能／未登録時は端末内保存、会員登録後は本人専用Firebaseへ移行
...
| 金種計算 | 紙幣硬貨の合計金額計算（日本円・全金種対象） | 4.2、15.15.3 |
...
各金種の枚数入力欄と合計金額の自動計算をG09画面に実装する。
