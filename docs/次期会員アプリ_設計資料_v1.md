...
| ID | 画面名 | 役割 |
|---|---|---|
| A01 | 会員登録 | 名前・ふりがな・メールアドレス・パスワードの登録フォーム |
...
## 12. 動画メモ・質問・YouTube検索登録・カテゴリ管理機能（旧仕様・適用範囲を下記で限定）

現行アプリに存在する「視聴動画メモ保存」「動画に関する質問」「YouTube検索・登録」「2段階カテゴリ」機能を、次期アプリへ共通仕様として引き継ぐ。本節はiPhone/Android共通の設計であり、実装・GitHubへの反映は行わない。

> **次期会員アプリの仕様確定（2026-08-10）**: 次期会員アプリで実装対象とする動画は、コミュニティが配信するVimeo動画に限定する。Vimeo動画の視聴、動画メモの編集・保存・削除、および会員UID単位のFirebase同期を対象とする。YouTube検索、YouTube動画の個人登録、個人動画の動画メモ、個人動画バックアップ、2段階カテゴリ管理は仕様変更により対象外とし、次期会員アプリへ再導入しない。動画質問機能はVimeo配信動画を対象として実装する。
...
- 会員登録では名前・ふりがな・メールアドレス・パスワードを必須入力とする。Firebase AuthのUIDをキーとして、既存会員データと互換性のある `memberPrivate/{uid}` に `uid`、`email`、`name`、`furigana`、`createdAt`、`updatedAt` を保存する
- 開発ビルドは開発用Firebaseプロジェクトのみに保存し、本番会員データは複製しない。本番リリース時は既存Firebaseプロジェクトと同じパス・フィールド形式を使用して既存会員データを継続利用する
...
**クライアント受信契約**: カスタムURLは、お知らせ`orgportalnext://announcements/{announcementId}`、会員投稿への返信`orgportalnext://posts/{postId}`、動画質問への回答`orgportalnext://video-questions/{questionId}`、イベント`orgportalnext://events/{eventId}`とする。コミュニティ配下の通知は`communityId`クエリを付ける。FCM data payloadは`notificationType`（`announcement`/`admin_reply`/`video_question_answer`/`event`）、種類別ID、`communityId`、または同じURLを格納した`deepLink`を使用する。既存の`orgportalnext://support/messages/{messageId}`と`support_message`は互換維持する。
...
|---|---|---|---|---|
| Guestホーム | 実装・単体テスト済み | 実装・単体テスト済み | ツール順序（6項目）と利用可否を両OSで検証 | ☑ |
...
| お知らせ・既読管理 | 完了 | 完了 | 作成済み | ☑ |
| 会員投稿・公開投稿 | 実装・単体テスト済み | 実装・単体テスト済み | 公開投稿のGuest閲覧、承認済み会員本人のCRUD、既存memberPosts形式、管理者返信・未読状態を両OSで検証 | ☑ |
| 使い方マニュアル | 追加済み | 追加済み | 目次・表示内容・遷移を両OSで検証 | ☑ |
| 動画視聴・メモ・質問 | 未着手 | 未着手 | 未作成 | ☐ |
| YouTube検索・登録・カテゴリ | 仕様変更で除去 | 仕様変更で除去 | 対象外（除去機能） | ☑ |
| インターネットラジオ | 未着手 | 未着手 | 未作成 | ☐ |
| イベント予約 | 未着手 | 未着手 | 未作成 | ☐ |
| コミュニティ接続・切替・サーフィン | 公開コミュニティ一覧・検索・詳細・参加導線、コード・QR参加申請、承認状態表示、承認済みコミュニティ切替、権限を持つ運営者の申請一覧・承認・却下を実装・単体テスト済み | 公開コミュニティ一覧・検索・詳細・参加導線、コード・QR参加申請、承認状態表示、承認済みコミュニティ切替、権限を持つ運営者の申請一覧・承認・却下を実装・単体テスト済み | Guestは公開フラグ付きコミュニティだけを閲覧でき、参加申請時はログインへ案内。コード単体・招待URLの共通解析、本人申請と状態改変防止、管理権限・監査記録、切替・元のコミュニティへ戻る動作を両OSで検証 | ☑ |
| 友達情報・交流履歴帳 | 実装済み（端末内） | 実装済み（端末内） | 作成済み | ☑ |
| コミュニティ運営 | 参加申請一覧・承認・却下、監査記録、Owner向け管理者コンソール、管理者追加・無効化、会員利用停止を実装 | 参加申請一覧・承認・却下、監査記録、Owner向け管理者コンソール、管理者追加・無効化、会員利用停止を実装 | Ownerが承認済み会員を氏名・ふりがな・メール・UIDで検索して管理者へ追加できること、委譲管理者の承認、無効化、会員利用停止を両OSで実機検証 | ☐ |
| 公開投稿・フォロー・ファン | 未着手 | 未着手 | 未作成 | ☐ |
| 有料運営プラン | 未着手 | 未着手 | 未作成 | ☐ |
| アカウント・セキュリティ | 開発用Firebase接続・会員情報保存・ログイン・ログアウト・生体認証再ログインを実装・単体テスト済み | 開発用Firebase接続・会員情報保存・ログイン・ログアウト・Face ID/Touch ID再ログインを実装・単体テスト済み | 名前・ふりがな・メール・パスワードの登録検証、メール認証、ログイン、再設定、`memberPrivate/{uid}`互換保存、ログアウト後の生体認証再ログインを両OSで検証。パスワードは保存せず、更新トークンをKeychain／Keystore暗号化領域へ保存 | ☐ |
| プッシュ通知・画面遷移 | Firebase Messagingによるトークン登録・更新・削除、4種類＋既存サポートのディープリンク、コミュニティ自動切替を実装・単体テスト済み | Firebase Messagingによるトークン登録・更新・削除、4種類＋既存サポートのディープリンク、コミュニティ自動切替を実装・単体テスト済み | `memberPrivate/{uid}/fcmTokens/{tokenId}`保存、Debug環境ガード、M03/M05/V14/M09遷移を両OSで検証 | ☑ |
| サポート・診断 | 未着手 | 未着手 | 未作成 | ☐ |
| コミュニティ別機能設定 | 未着手 | 未着手 | 未作成 | ☐ |
| 利用状況記録 | 実装・単体テスト済み | 実装・単体テスト済み | Domain Model/Firestore変換、P04オプトアウト時の送信停止、本人以外のRules拒否、定義外本文フィールド拒否を両OS・Emulatorで検証 | ☑ |
| 予算・決算 | 未着手 | 未着手 | 未作成 | ☐ |

### 15.3 各機能を利用できるアプリ段階

#### 15.3 補足（本実装フェーズ）
- 動画関連で「除去対象」となった機能は、以下を両OSで共通除去済みです。
  - YouTube検索（`youTubeSearch` の導線）
  - 個人登録動画（`PersonalVideo` / `PersonalVideoRepository` / `VideoMemo` 系）
  - 個人動画バックアップ（`PersonalVideoBackupService` / `AppBackupPersonalVideo` / `AppBackupVideoMemo`）
- `personal_youtube` の固定値参照（除去済み）
- 2026-08-09 時点での除去後確認（CI前提チェック用）
  - Android: `rg` で `PersonalVideo|VideoMemo|PersonalVideoBackupService|AppBackupPersonalVideo|AppBackupVideoMemo|youTubeSearch|personalVideos|videoQuestion|VideoQuestion|personal_youtube` を検索し、`NextApp/Android` 配下でヒットなし
  - iOS: 同様の `rg` で `NextApp/iOS/OrgPortalNext`（`Tests` 除外）を検索し、`PersonalVideo|VideoMemo|PersonalVideoBackupService|AppBackupPersonalVideo|AppBackupVideoMemo|youTubeSearch|personalVideos|videoQuestion|VideoQuestion|personal_youtube` でヒットなし
- `video質問` もこのフェーズで除去対象の実装・導線含めて除去済み
- Android/iOS 双方とも `personal_youtube` の残存はなし

| 機能 | Guest | Member（コミュニティ未参加） | Member（参加済み） | Creator/Owner/Manager |
|---|---|---|---|---|
| 予定・日記・金種計算・会議録音・SNS投稿補助 | ○ | ○ | ○ | ○ |
| 動画メモ・登録動画・カテゴリ管理・リピート再生 | 仕様変更で除去 | 仕様変更で除去 | 仕様変更で除去 | ☑ |
| 動画質問送信 | 不可 | 不可 | 不可 | 不可 |
| 動画質問への回答 | 不可 | 不可 | 不可 | 不可 |
| 友達情報・交流履歴帳 | ○（端末内） | ○ | ○ | ○ |
| 予算・決算（個人用） | ○（端末内） | ○ | ○ | ○ |
...
| 区分 | データ | 保存先パターン |
|---|---|---|
| 本人専用（他者・管理者とも共有しない） | 予定、日記、友達情報・交流履歴帳、SNSカスタムリンク、予算・決算、会議録音関連、生体認証設定 | `memberPrivate/{uid}/...` |
...
| `memberPrivate/{uid}/fcmTokens/{tokenId}` | 次期会員アプリのFCM端末トークン（本人専用） |
...
#### 15.15.24 実装時に判明した必須運用・データ契約

今回のAndroid/iPhone実機確認で、画面仕様だけでは動作を保証できない必須条件が判明した。以下を両OS共通の確定仕様として扱う。

- **Firebase開発環境の統一**: 開発版の接続先はFirebaseプロジェクト `kome-org-portal-next-dev` に統一する。iPhoneの`FIREBASE_PROJECT_ID`と`FIREBASE_WEB_API_KEY`、Androidの`BuildConfig.FIREBASE_PROJECT_ID`と`BuildConfig.FIREBASE_WEB_API_KEY`、Firebase CLIの`.firebaserc`は同一の開発プロジェクトを参照する。開発版から本番プロジェクトへ接続・書き込みを行わない。
- **会員登録とパスワード再設定**: Firebase Authenticationのメール/パスワード認証を有効化する。パスワード再設定はIdentity Toolkitの`accounts:sendOobCode`へ`requestType: PASSWORD_RESET`とメールアドレスを送信する。アプリの成功表示はFirebaseが送信要求を受け付けたことを示すもので、受信箱への到達は保証しない。Firebaseのパスワード再設定テンプレート、送信元、迷惑メール対策を運用確認する。Firebaseエラーコードは画面に表示し、原因を特定できるようにする。
- **ログアウト**: ログアウト時は、アプリ内セッションのユーザーID、認証トークン、選択中コミュニティ、直前のコミュニティを消去し、Guest状態へ戻す。生体認証用のOS安全領域に保存したRefresh Tokenは、再ログイン導線を維持するため別管理とする。
- **生体認証ログイン**: 通常ログイン成功後にRefresh TokenをiPhoneのKeychain、AndroidのKeystore等へ保存し、iPhoneはFace ID/Touch ID、AndroidはBiometricPromptで認証する。生体認証に失敗しても通常のメール/パスワードログインを利用できる。
- **コミュニティ参加申請の保存先**: 参加申請は`organizations/{organizationId}/members/{uid}`へ保存する。`status`は`pending`、`approved`、`rejected`のいずれかとし、申請者の`uid`または`userId`、`communityId`、申請者情報、作成・更新日時を保存する。
- **管理者権限の保存先**: 参加申請を確認・承認・却下できる権限は、通常のメンバー情報とは分離し、`organizations/{organizationId}/admins/{uid}`で管理する。`isActive: true`かつ、`role: owner`、または`permissions`に`memberReview`を含む場合に申請審査権限を持つ。管理者レコードを作成しただけでは、現在のコミュニティは自動選択されないため、管理者自身も対象コミュニティの`members/{uid}.status`を`approved`にする。
- **管理者画面の表示条件**: 「参加申請の承認」は、（1）ログイン済み、（2）承認済みメンバーのコミュニティが現在選択されている、（3）対象コミュニティの`admins/{uid}`が有効で審査権限を持つ、のすべてを満たした場合に表示する。
- **Firestoreコレクション全体検索インデックス**: Android/iPhoneは`members`サブコレクションを横断し、`uid`で現在ユーザーの参加コミュニティを検索する。この検索には`collectionGroup: members`、`fieldPath: uid`、`queryScope: COLLECTION_GROUP`の単一フィールドインデックスが必須である。インデックス定義は`firebase/firestore.indexes.json`で管理し、開発・本番プロジェクトへ環境ごとにデプロイする。インデックス未作成時は、Firestoreに承認済みデータが存在していてもアプリ上の参加コミュニティ一覧が空になる。
- **両OSの確認手順**: Firebase上のデータ更新後は、両OSでログアウト・再ログインし、「つながる」を開いて対象コミュニティを選択・更新する。Firestoreインデックスの作成中は一覧取得を完了できないため、インデックスが`READY`になってから確認する。
- **管理者コンソール**: 独立した管理者アプリを別バイナリで用意するのではなく、会員アプリ内でOwnerまたは委譲された管理権限を持つ利用者だけに管理機能を表示する。Ownerは対象コミュニティの承認済み会員を氏名・ふりがな・メールアドレス・UIDで検索し、候補の「管理者に追加」から`admins/{uid}`を登録できる。登録済み管理者は無効化でき、Owner自身は自分の権限を無効化できない。会員管理では承認済み会員を利用停止できる。
- **管理者追加の対象**: 管理者追加候補は対象コミュニティの`members/{uid}.status == approved`の会員に限定する。未承認会員を管理者にする場合は、先に参加申請を承認する。管理者の追加・無効化はコミュニティ単位で行い、別コミュニティの権限へ影響させない。
- **Android生体認証**: Androidは`BiometricPrompt`を使用し、端末差による顔認証の除外を避けるため`BIOMETRIC_WEAK`を許容する。生体認証未登録、非対応、認証失敗、更新トークン期限切れは画面へ理由を表示し、通常のメールアドレス・パスワードログインを代替手段として残す。通常ログイン後にログアウトすると、生体認証ログイン導線を利用できる。
- **Security Rulesの開発環境デプロイ**: 開発プロジェクト`kome-org-portal-next-dev`へ、会員自身の申請、Owner/権限保持者の審査、管理者管理、承認済み会員の読み取りを許可するRulesをデプロイ済み。本人が承認済み申請を`pending`へ戻す更新は拒否する。公開前には本番プロジェクトのRulesを別途レビューしてデプロイする。
...
### 15.15.25 Vimeo配信動画のメモ
- 会員アプリは、コミュニティに公開されたVimeo配信動画のみを対象に、動画ごとのメモを編集・保存できる。
- Android/iOSとも、端末内保存を表示用キャッシュとして使用し、Firebaseの `memberPrivate/{uid}/videoMemos` へ会員UID単位で同期する。同じ会員アカウントで別端末へログインした場合もメモを引き継ぐ。
- メモが空の状態で保存した場合は、その動画の端末内メモを削除する。
- YouTube検索・個人動画・個人動画バックアップ機能は再導入しない。Vimeo配信動画の視聴とメモ機能は、動画質問機能とは分離して扱う。
- Vimeo配信動画はAndroid WebView/iOS WKWebViewでアプリ内再生し、再生位置を取得して質問の `playbackSeconds` として保存する。
- 管理者コンソールでは登録済みVimeo動画一覧から動画を選択し、タイトル・説明・Vimeo情報を編集して公開／非公開を切り替えられる。
