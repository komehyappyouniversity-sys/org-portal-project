...
| 有料運営プラン | 未着手 | 未着手 | 未作成 | ☐ |
| アカウント・セキュリティ | 開発用Firebase接続・会員情報保存・単体テスト済み | 開発用Firebase接続・会員情報保存・単体テスト済み | 名前・ふりがな・メール・パスワードの登録検証、メール認証、ログイン、再設定、`memberPrivate/{uid}`互換保存を両OSで検証。ログアウト・退会・生体認証は後続タスク | ☐ |
