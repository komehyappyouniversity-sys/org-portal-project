...
| インターネットラジオ | 配信開始前は再生不可、配信済み番組は再生可能であること。音声は画面ロック中・バックグラウンドでも継続し、iOSのロック画面／コントロールセンターとAndroidの通知領域から一時停止・再開・停止できること。電話着信等の音声割り込みとイヤホン切断時は自動で一時停止し、割り込み終了時はOSが許可した場合のみ再開すること。一時停止・停止・30秒ごとに`memberPrivate/{uid}/radioPlaybackRecords`へ再生位置を保存し、同じ番組を次回開いたとき保存位置から再開すること。画面上の操作名と「再生中」「一時停止中」の状態表示は両OSで同じであること。動画のバックグラウンド再生は対象外のままとする |
