...
| 使い方マニュアル（M06） | Guestは`sharedManuals`の公開中データのみ、承認済み会員はそれに加えて選択中コミュニティの`organizations/{organizationId}/manuals`を表示する。両コレクションを`sortOrder`昇順（同値はアプリ共通→コミュニティ専用→ID順）で表示し、`isPublished == false`は一覧・詳細に出さない。本文、画像一覧と拡大表示、PDFを開く、外部リンクを開く、読み込み中、0件、通信エラー、再試行の文言と挙動が両OSで同じであること |
...
| 利用状況記録 | iOS/Androidとも、動画詳細表示・再生開始・30秒単位の再生位置・視聴完了・ラジオ再生開始だけを `memberPrivate/{uid}/usageLogs` へ記録する。投稿本文、友達情報、会議音声、メモ本文などは送信しない。P04の「任意の利用状況記録を停止」がONの場合は送信前ゲートで全UsageLogを停止する。端末設定はiOSがAppStorage/UserDefaults、AndroidがDataStore。通常イベントは`occurredAt + 90日`の`expiresAt`にTTLを設定し、Rulesで本人以外の読み書きと定義外フィールドを拒否する |
...
- 処理中と多重操作防止
- 重要操作の確認と成功・失敗メッセージ
- 利用状況記録のオプトアウト状態（ONの場合はUsageLog送信なし）
